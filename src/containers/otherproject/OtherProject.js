    ...
    return (
        <ScrollTrigger onEnter={() => setIsVisible(true)}>
            <div className={`otherproject-container ${isVisible ? 'visible' : ''}`}>
                ...
                <div className='otherproject-info'>
                    <span className='otherproject-title'>{label.title}</span>
                    <span className='otherproject-description'>{label.description}</span>
                    <div className='otherproject-technologies'>
                        {label.technologies.map((technology, index) => (
                            <TechnologyDesc label={technology} leftAligned={true}/>
                        ))}
                    </div>
                </div>
            </div>
